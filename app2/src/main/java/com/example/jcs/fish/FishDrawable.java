...
public class FishDrawable extends Drawable {
	...
	private void makeBody(Canvas canvas, float headRadius) {
		...
		//计算的角度是90，为正方向，currentValue的默认值是0
		headPoint = calculatPoint(middlePoint, BODY_LENGHT / 2,mainAngle);   //计算headPoint  middlePoint是重心
		//画头
	...
	private void makeFins(Canvas canvas, PointF startPoint, int type, float fatherAngle) {
		...
		mPath.reset();
		mPath.moveTo(startPoint.x, startPoint.y);   //绘制路径  p0
		PointF endPoint = calculatPoint(startPoint, FINS_LENGTH, type == FINS_RIGHT ? fatherAngle - finsAngle-180 : fatherAngle + finsAngle+180);  //p2
		PointF contralPoint = calculatPoint(startPoint, FINS_LENGTH * 1.8f, type == FINS_RIGHT ?
				fatherAngle - contralAngle - finsAngle : fatherAngle + contralAngle + finsAngle);   //p1
		mPath.quadTo(contralPoint.x, contralPoint.y, endPoint.x, endPoint.y);
		mPath.lineTo(startPoint.x, startPoint.y);
		mPaint.setColor(Color.argb(FINS_ALPHA, 244, 92, 71));   //设置画笔的颜色
		canvas.drawPath(mPath, mPaint);
		mPaint.setColor(Color.argb(OTHER_ALPHA, 244, 92, 71));
