...
public class MyIndicator extends RelativeLayout {
    ...
    public MyIndicator(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        for (int i = 0; i < mList.length; i++) {
            mListTitle.add(mList[i]);    //获取title集合
        }
        array.recycle();
    }
    ...
    @Override
    protected void onLayout(boolean changed, int l, int t, int r, int b) {   //layout布局
    ...
    private void initView() {
        ...
        mTextList.clear();
        //获取每个textview布局所占的宽度
        mContWidth = getWidth() / mListTitle.size();   //每个textView占据的大小
        //添加线
        ...
        if (isFull) {
            ...
            mWeight = (int) (setLineLength(mListTitle.get(mSelected)) * mBai);   //计算线的长度
        ...
        //获取上一次所在位置
        mEndAddress = mContWidth * mSelected + (mContWidth - mWeight) / 2;   //获取线的位置
        //添加textview
